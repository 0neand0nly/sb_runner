...
def run_spotbugs_on_jar_files(jar_files, output_directory):
    for jar_file in jar_files:
        report_filename = os.path.basename(jar_file).replace('.jar', '.xml')
        # Ensure that the XML output is directed to a file in the specified output directory
        output_path = os.path.join(output_directory, report_filename)
        command = [
            'java', '-jar', spotbugs_path,
            '-textui', f'-xml:withMessages={output_path}',
            '-low',
            jar_file
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument('output_directory', help='The directory where the SpotBugs reports will be saved.')
    ...
    run_spotbugs_on_jar_files(jar_files, args.output_directory)
